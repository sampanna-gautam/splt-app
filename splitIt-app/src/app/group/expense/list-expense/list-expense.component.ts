...
export class ListExpenseComponent implements OnInit {
  ...
  expenseFilter(): (data: any, filter: any) => boolean {
    const filterFunction = (data: any, filter: any): boolean => {
      ...
      const isExpenseNameMatch = data.expenseName
        .toLowerCase()
        .includes(searchText);
      const isPayerNameMatch = data.payerName
        .toLowerCase()
        .includes(searchText);
      ...
      return (
        (filter
          ? isExpenseNameMatch || isPayerNameMatch || isAmountMatch
          : true) && (dateCheck ? isDateInRange : true)
      );
    };
