...
const verifyEmail = require("./functions/verifyEmail.js");
...
const uri =
  "mongodb+srv://gautamsampanna11:ilovebhargav@cluster0.sqgbo.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0";
...
app.post("/api/register", async (req, res) => {
  try {
    ...
    if (!(await verifyEmail(email))) {
      return res
        .status(400)
        .json({ message: "That email address is not a valid email address!" });
    }
    ...
    if (existingUser) {
      return res.status(400).json({
        message: "Email address is already in use. Please login to continue.",
      });
    }
  ...
  try {
    ...
    if (!isPasswordValid) {
      return res.status(401).json({
        ...
        suggestion:
          "Oops! Password slipped your mind? No biggie, happens to everyone! Just tap that reset button and let's work our magic to get you back in action",
      });
    }
...
app.post("/api/reset-password", async (req, res) => {
  try {
    ...
    if (!existingUser) {
      return res.status(400).json({
        message:
          "Email not in our system. Please register to continue or check email entered is correct.",
      });
    }
