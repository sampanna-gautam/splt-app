...
async function calculateAndUpdateBalances(group, expense, operation) {
  const payer = group.members.find(
    (member) => member.memberId.toString() == expense.payer.toString(),
  );
  const participantsList =
    expense.participants instanceof Map
  ...
  for (const [participantId, participantAmount] of participantsList.entries()) {
    const member = group.members.find(
      (member) => member.memberId == participantId,
    );
    ...
    const unroundedResult =
      operation === "add"
        ? member.memberBalance - participantAmount
        : member.memberBalance + participantAmount;
  ...
  const unroundedResult =
    operation === "add"
      ? payer.memberBalance + expense.amount
      : payer.memberBalance - expense.amount;
  const roundedResult = parseFloat(unroundedResult.toFixed(2));
...
function settleGroupDebts(group, operation) {
  ...
  while (creditors.length > 0) {
    ...
    let creditorMember = group.members.find(
      (member) => member._id == creditor._id,
    );
    creditorMember.memberBalance =
      operation === "add"
        ? creditorMember.memberBalance - creditor.memberBalance
        : creditorMember.memberBalance + creditor.memberBalance;
    creditorMember.memberBalance = Number(
      creditorMember.memberBalance.toFixed(2),
    );
    creditors.shift();
  }
...
function settleDebts(members) {
  const debtors = members
    .filter((member) => member.memberBalance < 0)
    .sort((a, b) => a.memberBalance - b.memberBalance);
  const creditors = members
    .filter((member) => member.memberBalance > 0)
    .sort((a, b) => b.memberBalance - a.memberBalance);
  ...
  while (debtors.length > 0 && creditors.length > 0) {
    ...
    debtor.memberBalance = parseFloat(
      (debtor.memberBalance + absoluteBalance).toFixed(2),
    );
    creditor.memberBalance = parseFloat(
      (creditor.memberBalance - absoluteBalance).toFixed(2),
    );
...
async function handleExpenseDeletion(req, res) {
  try {
    ...
    if (!expense.groupId || !group) {
      return res
        .status(404)
        .json({ message: "Expense Group couldn't be linked" });
    }
...
async function handleExpenseUpdate(req, res) {
  try {
    ...
    if (!oldExpense.payer || !payerUser) {
      return res
        .status(404)
        .json({ message: "Previous Expense Payer not found" });
    }
