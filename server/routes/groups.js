  ...
  try {
    ...
    if (existingGroup) {
      return res.status(400).json({
        message:
          "Group name is already in use. Please choose a different name.",
      });
    }
  ...
  try {
    ...
    if (user) {
      if (!group.members.some((member) => member.memberId.equals(user._id))) {
        ...
        return res
          .status(404)
          .json({ message: "User already present in group." });
      }
    } else {
      return res
        .status(404)
        .json({ message: "User not found. Please re-check the email" });
    }
...
router.post("/:groupId/transaction/:transactionId", async (req, res) => {
  try {
    ...
    debtor = group.members.find(
      (member) =>
        member.memberId.toString() == group.balance[transactionIndex].from,
    );
    ...
    creditor = group.members.find(
      (member) =>
        member.memberId.toString() == group.balance[transactionIndex].to,
    );
