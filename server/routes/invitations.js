...
router.post("/send", async (req, res) => {
  try {
    ...
    if (
      user.invitations.some((invitation) => invitation.groupId.equals(groupId))
    ) {
      return res.status(409).json({
        message: "User already invited to this group.",
        type: "user_already_invited",
        suggestion:
          "Looks like someone's quite popular or has a lot to payback.",
      });
    }
...
router.post("/accept", async (req, res) => {
  try {
    ...
    if (invitation.recipientId.toString() !== user._id.toString()) {
      ...
      return res
        .status(403)
        .json({ message: "Invalid user for this invitation" });
    }
